...
int block(void*pv){ int p=T(); void**v=pv; int olen=*(long*)v;
  if(s(v,"{")&&stmts(v)&&s(v,"}"))return 1; trimv(v,olen); S(p);  return 0;}
int funcall(void*pv){ int p=T(); void**v=pv; int olen=*(long*)v;
...
int prog(void*pv){ int p=T(); void*v[1024]={(void*)1,"||||prog"}; int olen=*(long*)v;
